...
******
To begin, the number of golfers from each year is determined. The barplot shows
...
effects of all variables even if a variable is correlated with the single best predictor.
...
statistical models to predict winners on the PGA tour can be constructed. The models 
...
Tour in 2019, the model correctly identified only 22% of the winners, which is far less impressive than CCR suggests. These seemingly contradictory results occur because the
...
win a tournament; however, the models were good at selecting the top ranked golfers in 
